...
export class PlanComponent implements OnInit, OnDestroy, AfterViewInit {
            ...
            .subscribe({
                next: (seatMap: SeatMap) => {
                    ...
                    this.useVirtualScroll =
                        this.virtualScrollService.shouldUseVirtualScroll(
                            seatMap.rows,
                            seatMap.columns
                        );
                    ...
                    if (this.useVirtualScroll) {
                        console.log(
                            `Large seat map detected: ${seatMap.rows}x${
                                seatMap.columns
                            } (${
                        ...
                        console.log(
                            `Standard seat map: ${seatMap.rows}x${
                                seatMap.columns
                            } (${
                                seatMap.rows * seatMap.columns
                ...
                error: (err) => {
                    ...
                    this.error =
                        'خطا در بارگذاری نقشه صندلی‌ها. لطفا دوباره تلاش کنید.';
                    this.loading = false;
                    this.cdr.detectChanges();
                },
            });
    ...
    onSeatClick(rowIndex: number, colIndex: number): void {
        ...
        if (seatStatus === SeatStatus.RESERVED) {
            console.log(
                `Seat (${colIndex}, ${rowIndex}) is reserved and cannot be selected`
            );
            return;
        }
    ...
    getSeatStatus(rowIndex: number, colIndex: number): SeatStatus {
        ...
        return this.selectedSeats.has(seatKey)
            ? SeatStatus.SELECTED
    ...
    purchaseSelectedSeats(): void {
        ...
        selectedCoordinates.forEach((coord, index) => {
            ...
            console.log(
                `Purchasing seat at coordinates (${coord.x}, ${coord.y})`
            );
                ...
                .subscribe({
                    next: (response: TicketPurchaseResponse) => {
                        ...
                        if (response.success) {
                            successfulPurchases++;
                            console.log(
                                `Purchase successful for seat (${coord.x}, ${coord.y}):`,
                                response
                            );
                            ...
                            if (
                                ...
                                this.seatMap.seats[coord.y][coord.x] !==
                                    undefined
                            ) {
                                this.seatMap.seats[coord.y][coord.x] =
                                    SeatStatus.RESERVED;
                            ...
                            console.log(
                                `Purchase failed for seat (${coord.x}, ${coord.y}):`,
                                response.message
                            );
                        }
                    ...
                    error: (err) => {
                        ...
                        console.error(
                            `Error purchasing seat (${coord.x}, ${coord.y}):`,
                            err
                        );
     ...
     */
    private handlePurchaseCompletion(
        successCount: number,
        failures: string[]
    ): void {
        ...
        if (successCount > 0 && failures.length === 0) {
            ...
            this.purchaseError = `خطا در خرید ${
                failures.length
            } صندلی: ${failures.join(', ')}`;
        } else {
    ...
    getRowOffset(index: number): number {
        return this.virtualScrollService.getRowOffset(
            this.visibleRows.start + index
        );
    }
    ...
    isColumnVisible(colIndex: number): boolean {
        ...
        return (
            colIndex >= this.visibleCols.start &&
    ...
    getVisibleSeats(row: number[]): { seat: number; index: number }[] {
        ...
        for (
            let i = this.visibleCols.start;
            i < Math.min(this.visibleCols.end, row.length);
            i++
        ) {
